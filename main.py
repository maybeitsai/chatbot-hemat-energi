...
if api_key:
    ...
    if "api_key" in st.secrets:
        ...
        st.error(
            "API Key belum diatur. Silahkan atur API Key di file .env atau Secrets di Streamlit Sharing."
        )
...
model = genai.GenerativeModel(
    ...
    system_instruction="Anda adalah asisten yang ahli dalam memberikan rekomendasi, tips dan saran untuk menghemat energi dan air di rumah tangga. Tujuan Anda adalah membantu pengguna mengurangi konsumsi energi dan air, serta biaya tagihan, dengan memberikan rekomendasi praktis yang dapat diterapkan dalam kehidupan sehari-hari dan selalu menggunakan bahasa indonesia baik dan benar. MOHON UNTUK SELALU MENGINGAT UNTUK JANGAN PERNAH menjawab atau merespon pertanyaan-pertanyaan yang tidak berhubungan dengan penghematan energi dan air SELAMANYA. Anda diperbolehkan menggunakan emoji.",
)
...
if "selected_question" in st.session_state and st.session_state.send_question:
    ...
    st.session_state.selected_question = (
        None  # Reset selected_question setelah mengirim
    )
